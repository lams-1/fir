...
- createWorker est appelé avec le modèle distant `engJost-final3`.
- Worker et Core WASM servis localement depuis public/tesseract.
- Langue (traineddata) chargée depuis URL externe : `https://files.kubuxu.com/foxhole/tesseract/`.
- Optimisation : Cache IndexedDB activé par défaut.
- Fichier requis : aucun fichier de langue local (téléchargement au runtime).
- Paramètres OCR :
...
- Vérifier que les assets OCR sont chargés :
  ...
  - (Modèle de langue chargé depuis l'URL distante)
...
- Vérifier que les assets OCR (JS/WASM) sont bien accessibles : /tesseract/worker.min.js, /tesseract/tesseract-core-simd.wasm.js.
...
- Tesseract.js v5 requiert l’API createWorker('engJost-final3', ...).
- Les chemins d’assets OCR (worker/core) doivent être servis par Vite (public/tesseract).
- Le modèle de langue est distant (Kubuxu) pour bénéficier des optimisations spécifiques Foxhole.

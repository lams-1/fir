...
## TFJS in the Frontend (Overview)

- TFJS is loaded in the browser via CDN in [index.html](index.html), [debug.html](debug.html), and [specs.html](specs.html).
- Model URLs are assembled in [includes/main.js](includes/main.js) and passed to [includes/frontend.mjs](includes/frontend.mjs).
- Inference runs in [includes/screenshot.mjs](includes/screenshot.mjs) via `tf.loadGraphModel(...)` and `Screenshot.process(...)`, operating on `canvas` image data.

## Training Pipeline (Overview)

The end-to-end training flow is orchestrated by [build.sh](build.sh):

1. Parse the Foxhole catalog from the game data (via `catalog/parse.js`).
2. Generate training icon images in `catalog/training/` (via `catalog/generate_training.js`).
3. Train the icon classifier with TensorFlow/Keras (via [trainer/train.py](trainer/train.py)).
4. Convert the SavedModel to TFJS format with `tensorflowjs_converter` and write it to `foxhole/<version>/classifier/`.

The quantity model uses synthetic images generated by [trainer/generate_quantities.sh](trainer/generate_quantities.sh).

## Icon Sourcing for Training

Icons are taken from the Foxhole installation (`$WAR_LOCATION`) and rendered into training samples by [catalog/generate_training.js](catalog/generate_training.js):

- The script loads the primary icon path from each catalog entry (`Icon`).
- If mods exist, it tries `mod-files/<modName>/<Icon>` first.
- It can overlay subtype corner icons (`SubTypeIcon`) and a crate overlay.
- Multiple sizes and minor offsets are generated to improve robustness.
